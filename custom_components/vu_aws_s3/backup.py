"""Backup platform for the AWS S3 Folder integration."""
...
from .const import CONF_BUCKET, CONF_PATH, DATA_BACKUP_AGENT_LISTENERS, DEFAULT_PATH, DOMAIN
...
async def async_get_backup_agents(
    ...
    entries: list[S3FolderConfigEntry] = hass.config_entries.async_loaded_entries(DOMAIN)
    return [S3FolderBackupAgent(hass, entry) for entry in entries]
...
class S3FolderBackupAgent(BackupAgent):
    """Backup agent for the S3 Folder integration."""

    domain = DOMAIN

    def __init__(self, hass: HomeAssistant, entry: S3FolderConfigEntry) -> None:
        """Initialize the S3 Folder agent."""
        super().__init__()
        self._client = entry.runtime_data
        self._bucket: str = entry.data[CONF_BUCKET]
        self._base_path: str = entry.data.get(CONF_PATH, DEFAULT_PATH)
    ...
    def _get_full_key(self, key: str) -> str:
        """Get the full S3 key by combining the base path with the given key."""
        return join_path_elements(self._base_path, key)
    ...
    async def async_download_backup(
        ...
        **kwargs: Any,
        ...
        # 使用基礎路徑計算完整的檔案路徑
        full_key = self._get_full_key(tar_filename)

        response = await self._client.get_object(Bucket=self._bucket, Key=full_key)
    ...
    async def async_upload_backup(
        ...
        **kwargs: Any,
        ...
        # 使用基礎路徑計算完整的檔案路徑
        full_tar_filename = self._get_full_key(tar_filename)
        full_metadata_filename = self._get_full_key(metadata_filename)

        try:
            if backup.size < MULTIPART_MIN_PART_SIZE_BYTES:
                await self._upload_simple(full_tar_filename, open_stream)
            else:
                await self._upload_multipart(full_tar_filename, open_stream)

            # Upload the metadata file
            metadata_content = json.dumps(backup.as_dict())
            await self._client.put_object(
                Bucket=self._bucket,
                Key=full_metadata_filename,
    ...
    async def async_delete_backup(
        ...
        **kwargs: Any,
        ...
        await self._client.delete_object(Bucket=self._bucket, Key=full_tar_filename)
        await self._client.delete_object(Bucket=self._bucket, Key=full_metadata_filename)
